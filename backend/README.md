...
NestShip is a commercial SaaS boilerplate built with a decoupled NestJS API and
...
| `APP_NAME`                       | `NestShip API`            | Service name used in API documentation and health responses       |
...
| `POSTGRES_DB`                    | `nestship`                | Local Compose database name                                       |
| `POSTGRES_USER`                  | `nestship`                | Local Compose database user                                       |
| `POSTGRES_PASSWORD`              | `nestship_local_password` | Local-only Compose database password                              |
| `POSTGRES_PORT`                  | `5432`                    | Loopback port exposed by Compose                                  |
| `DATABASE_URL`                   | Local NestShip URL        | PostgreSQL connection URL used by Prisma                          |
...
| `AUTH_REFRESH_COOKIE_NAME`       | `nestship_refresh`        | HttpOnly refresh cookie name                                      |
...
| `EMAIL_FROM_NAME`                | `NestShip`                | Friendly sender name                                              |
...
`owner@nestship.local`, `member@nestship.local`, and the `nestship-demo`
...
NestShip uses a polyrepo architecture. The frontend and backend are deployed
